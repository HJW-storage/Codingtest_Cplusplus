	...
	while (q.size()) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (wall_break == 0) {
				...
				else if (visited[next_row][next_col][1] == 0 && grid[next_row][next_col] == 1) {
					visited[next_row][next_col][1] = 1; // 방문처리
					q.push({ next_row, next_col, 1, cur_dist + 1 });
				}
			}
			// 벽은 부수고 온 경우. 
			else if (wall_break == 1) {
				if (visited[next_row][next_col][1] == 0 && grid[next_row][next_col] == 0) {
					visited[next_row][next_col][1] = 1; // 방문처리
			...
			//// 위의 조건별 문장을 한 문장으로 나타내면 다음과 같다. 단, 다음 if문 예외처리가 필요하다.
			//if (wall_break > 1) continue; // 해당 문제의 벽부수기는 최대 1이다. 
			//if (wall_break >= 0) {
			...
			//		}
			//	}
			//	else if (visited[next_row][next_col][wall_break] == 0 && grid[next_row][next_col] == 1) {
			//		visited[next_row][next_col][wall_break] = 1; // 방문처리
